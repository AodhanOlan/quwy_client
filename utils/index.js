...
function LoadHandler(o) {
    ...
    this.onload = ()=>{
        ...
        API.requestSeries(SCP.series).then(data=>{
            Reader.setSCP(SCP);
...
function ReaderAPI(o) {
    ...
    this.requestSeries = function(slug) {
        this.seriesRequest = fetch(this.seriesUrl + slug + '/').then(response=>response.json()).then(seriesData=>{
            seriesData = this.infuseSeriesData(seriesData);
            seriesData.chaptersIndex = Object.keys(seriesData.chapters).sort((a,b)=>parseFloat(a) - parseFloat(b));
            seriesData.volMap = {};
...
function themeHandler() {
    ...
    this.themeUpdated = ()=>{
        ...
        if (theme === 'Custom')
            ...
            this.setTheme('#272836', '#121223', '#487DE4', '#EEEEEE');
        else if (theme === 'Zaibatsu')
            this.setTheme('#1D1D1D', '#000000', '#BA1F1F', '#EEEEEE');
        else if (theme === 'Light')
            this.setTheme('#F1F4FF', '#FFFFFF', '#5889F0', '#2B2B2B');
        if (reset === true)
            this.resetCustom();
    }
    this.setTheme = (sidebar,reader,accent,text)=>{
        ...
        document.documentElement.style.setProperty("--accentSelectedInvert", (luma < 160 ? '#444444' : '#cccccc'));
        if (luma > 100) {
        ...
        if (Math.abs(r - rt) < 50 && Math.abs(g - gt) < 50 && Math.abs(b - bt) < 50) {
            if (luma > 200) {
...
function Setting(o) {
    ...
    this.set = function(value, silent, notip) {
        if (!this.checkOptionValidity(value))
            return null;
        this.setting = value;
    ...
    this.cycle = function(options, silent, notip) {
        ...
        case SETTING_VALUE_STEPPED:
            ...
            return this.set((index + 1 > options.length - 1) ? options[0] : options[index + 1], silent);
            break;
        }
        return null;
    }
    this.next = function(silent, notip) {
        ...
        if (this.options().includes(this.setting)) {
            ...
            if (targetIndex > this.options().length - 1)
                return;
...
function SettingsHandler() {
    ...
    this.query = function(qu, or) {
        if (or) {
            for (var key in qu) {
                ...
                if (qu[key][0] == '!') {
                    if (qu[key].substr(1) != setting.get())
                        return true;
                } else {
                    if (qu[key] == setting.get())
                        return true;
                }
            }
            return false;
            ...
            for (var key in qu) {
                ...
                if (qu[key]instanceof Array) {
                    ...
                    if (qu[key].includes(setting.get()))
                        continue;
                }
                if (qu[key][0] == '!') {
                    if (qu[key].substr(1) == setting.get())
    ...
    this.disable = (setting)=>{
        setting.disable();
        this.update(setting.addr, new SettingsPacket('disable',setting.addr));
    }
    this.enable = (setting)=>{
        setting.enable();
        this.update(setting.addr, new SettingsPacket('enable',setting.addr));
    }
    this.update = function(setting, packet) {
        if (!packet) {
            packet = new SettingsPacket('change',setting,this.getByAddr(setting).get());
        }
        if (packet.type == 'set')
            packet.type = 'change';
        packet.handled = true;
    ...
    this.settingUpdated = function(e) {
        if (e.type == 'change') {
            if (e.setting.postUpdate)
                e.setting.postUpdate(e.setting.get());
            this.refreshAll();
            if (localStorage)
    ...
    this.newSetting({
        addr: 'lyt.fit',
        prettyName: 'Page fit',
        options: ['none', 'all_limit', 'width_limit', 'height_limit', 'all', 'width', 'height'],
        ...
        condition: {
            'thm.theme': ['Custom'],
            'thm.primaryCol': ['!#3A3F44', 'or'],
            'thm.readerBg': ['!#272B30', 'or'],
            'thm.accentCol': ['!#B2DFFB', 'or'],
            'thm.textCol': ['!#EEEEEE', 'or']
        },
        type: SETTING_BUTTON
    }).newSetting({
        addr: 'lyt.spread',
...
function UI_Tooltippy(o) {
    ...
    this.set = function(text) {
        ...
        if (!this.attached || IS_MOBILE) {
            ...
            this.$.style.left = 'unset';
            this.$.style.right = Reader.imageView.$.offsetWidth / 2 - this.$.offsetWidth / 2 + 'px';
        }
    ...
    this.attach = function(element, text, align, offset) {
        ...
        if (align)
            element.setAttribute('data-tip-align', align);
...
function UI_Reader(o) {
    ...
    new KeyListener(document.body).attach('prevCh', ['BracketLeft'], e=>this.prevChapter()).attach('nextCh', ['BracketRight'], e=>this.nextChapter()).attach('prevVo', ['Comma'], e=>this.prevVolume()).attach('nextVo', ['Period'], e=>this.nextVolume()).attach('fit', ['KeyF'], e=>Settings.cycle('lyt.fit')).attach('layout', ['KeyD'], e=>Settings.cycle('lyt.direction')).attach('hide', ['KeyH'], e=>this.$.classList.toggle('o')).attach('sidebar', ['KeyS'], s=>Settings.cycle('apr.sidebar')).attach('pageSelector', ['KeyN'], s=>Settings.cycle('apr.selPinned')).attach('preload', ['KeyL'], s=>Settings.cycle('bhv.preload')).attach('spread', ['KeyQ'], s=>Settings.cycle('lyt.spread')).attach('spreadCount', ['KeyU'], s=>Settings.cycle('adv.spreadCount')).attach('spreadOffset', ['KeyI'], s=>Settings.cycle('adv.spreadOffset')).attach('share', ['KeyR'], s=>this.copyShortLink(s)).attach('enter', ['Enter'], s=>{
        if (this.SCP.page == this.SCP.lastPage)
            this.nextChapter();
    }
    ).attach('options', ['KeyO'], e=>Loda.display('settings')).attach('jump', ['KeyJ'], e=>Loda.display('jump'))
    new KeyListener(document.body).condition(()=>['width', 'width_limit'].includes(Settings.get('lyt.fit'))).attach('minus', ['Minus'], s=>Settings.prev('lyt.zoom', undefined, true)).attach('plus', ['Equal'], s=>Settings.next('lyt.zoom', undefined, true))
    new KeyListener(document.body).condition(()=>Settings.get('lyt.direction') == 'ltr').attach('prev', ['ArrowLeft'], e=>this.prevPage()).attach('next', ['ArrowRight'], e=>this.nextPage());
    new KeyListener(document.body).condition(()=>Settings.get('lyt.direction') == 'rtl').attach('prev', ['ArrowRight'], e=>this.prevPage()).attach('next', ['ArrowLeft'], e=>this.nextPage());
    ...
    this.controlFeatures = function(isFirstParty) {
        if (isFirstParty) {
            new KeyListener(document.body).attach('search', ['Ctrl+KeyF'], s=>{
                Loda.display('search')
            }
    ...
    this.setSCP = function(SCP) {
        if (SCP.series)
            this.SCP.series = SCP.series;
        if (SCP.chapter)
            this.SCP.chapter = SCP.chapter;
        if (SCP.page)
    ...
    this.fetchChapter = function(chapter, group) {
        return new Promise((resolve,reject)=>{
            ...
            if (!targetChapter.loaded[group] && targetChapter.pageRequest[group]) {
    ...
    this.initChapter = function(chapter, page, group) {
        ...
        if (!this.SCP.chapterObject.loaded[this.SCP.group] && this.SCP.chapterObject.images[this.SCP.group].length === 0) {
            this.SCP.chapterObject.pageRequest[this.SCP.group]().then((count)=>{
                delete this.SCP.chapterObject.pageRequest[this.SCP.group];
    ...
    this.getGroup = function(chapter, getDefault) {
        ...
        if (!getDefault) {
            if (Settings.get('misc.groupPreference') !== undefined && chapterObj.groups[Settings.get('misc.groupPreference')] !== undefined)
    ...
    this.displayPage = (page,dry)=>{
        ...
        if (HAS_LOCALSTORAGE && this.imageView.visiblePages && this.imageView.visiblePages.includes(this.SCP.lastPage) && !this.SCP.chapterObject.notice) {
    ...
    this.drawPreviews = (state)=>{
        ...
        this.previews.clear();
        if (state == true) {
            this.current.chapters[this.SCP.chapter].previews[this.SCP.group].forEach(preview=>{
    ...
    this.nextChapter = function() {
        ...
        if (this.SCP.chapter != this.SCP.lastChapter) {
            ...
            if (index < 0)
                throw new Error('Chapter advance failed: invalid base index.')
            this.initChapter(this.current.chaptersIndex[index + 1], 0)
        }
    }
    this.prevChapter = function(page) {
        ...
        if (this.SCP.chapter != this.SCP.firstChapter) {
            var index = this.current.chaptersIndex.indexOf('' + this.SCP.chapter);
            if (index < 0)
                throw new Error('Chapter stepback failed: invalid base index.')
            this.initChapter(this.current.chaptersIndex[index - 1], page || 0)
        }
    }
    ...
    this.enqueuePreload = images=>{
        images.filter(item=>item !== undefined).slice(0, 4).forEach((img,i)=>this._.preload_entity.children[i].src = img.url);
    }
    ...
    this.settingsRouter = function(o) {
        ...
        if (settings[o.setting])
            settings[o.setting](o.value);
    }
    ...
    new UI_MultiStateButton({
        node: this._.preload_button,
        setting: 'bhv.preload'
    }).S.biLink(Settings);
    new UI_MultiStateButton({
        node: this._.layout_button,
        setting: 'lyt.direction'
    }).S.biLink(Settings);
    new UI_MultiStateButton({
        node: this._.fit_button,
        setting: 'lyt.fit'
    }).S.biLink(Settings);
    new UI_MultiStateButton({
        node: this._.sel_pin_button,
        setting: 'apr.selPinned'
    }).S.biLink(Settings);
    new UI_MultiStateButton({
        node: this._.sidebar_button,
        setting: 'apr.sidebar'
    }).S.biLink(Settings);
    new UI_MultiStateButton({
        node: this._.previews_button,
        setting: 'apr.previews'
    }).S.biLink(Settings);
    new UI_MultiStateButton({
        node: this._.spread_button,
        setting: 'lyt.spread'
    }).S.biLink(Settings);
    ...
    , false)
    Tooltippy.attach(this._.chap_prev, 'Previous chapter [[]').attach(this._.chap_next, 'Next chapter []]').attach(this._.vol_prev, 'Previous volume [,]').attach(this._.vol_next, 'Next volume [.]').attach(this._.preload_button, 'Change preload [L]').attach(this._.layout_button, 'Change layout direction [D]').attach(this._.fit_button, 'Change fit mode [F]').attach(this._.sel_pin_button, 'Pin page selector [N]').attach(this._.sidebar_button, 'Show/hide sidebar [S]', 'right').attach(this._.previews_button.querySelector('.expander'), 'Show previews [P]').attach(this._.share_button, 'Copy short link [R]').attach(this._.search, 'Search the manga... [Ctrl]+[F]').attach(this._.jump, 'Jump to chapter... [J]').attach(this._.spread_button, 'Change two-page mode [Q]').attach(this._.settings_button, 'Advanced settings... [O]').attach(this._.download_chapter, 'Download chapter in the background')
    this.S.mapIn({
...
function UI_ReaderImageView(o) {
    ...
    new KeyListener(document.body,'hold').attach('slideDown', ['ArrowDown'], (e,frame)=>{
    ...
    document.onscroll = this._.image_container.onscroll = e=>{
        ...
        if (Settings.get('lyt.direction') != 'ttb')
            return true;
        Reader.stickHeader();
        var scrollTop = (e.target.scrollingElement) ? e.target.scrollingElement.scrollTop : undefined || e.target.scrollTop;
        ...
        if (window.test_element)
            window.test_element.style.top = st + 'px';
    ...
    this.updateWides = ()=>{
        if (Settings.get('lyt.direction') == 'ttb' && this.wrappers.current) {
            ...
            if (this.scroll.anchorTimeout)
                clearTimeout(this.scroll.anchorTimeout);
    ...
    this.drawImages = function(images, wides) {
        ...
        this.imageWrappers = this.imageWrappersMask.map(wrapper=>new UI_ReaderImageWrapper({
    ...
    this.selectPage = function(index, dry) {
        ...
        if (Settings.get('lyt.direction') == 'ttb') {
            ...
            if (Settings.get('bhv.resetScroll'))
                scroll(this.getScrollElement(), 0, 0);
        }
        var spreadCount = Settings.get('adv.spreadCount');
        ...
        if (spreadCount == 2) {
            Reader.enqueuePreload([this.imageList[index - 2], this.imageList[index - 1], this.imageList[index + 2], this.imageList[index + 3]]);
        } else {
            Reader.enqueuePreload([this.imageList[index - 1], this.imageList[index + 1]]);
        }
        this.S.out('event', {
            type: 'newPageIndex',
            data: index
    ...
    this.moveWrappers = (offset,snap)=>{
        if (!this.touch.affectedWrappers) {
            ...
            if (this.wrappers.left)
                this.touch.affectedWrappers.push(this.wrappers.left);
            this.touch.affectedWrappers.push(this.wrappers.current);
            if (this.wrappers.right)
                this.touch.affectedWrappers.push(this.wrappers.right);
        }
        if (snap) {
        ...
        if (this.wrappers.left)
            this.wrappers.left.$.style.transform = `translateX(${offset * 100}%)`;
        this.wrappers.current.$.style.transform = `translateX(${offset * 100}%)`;
        if (this.wrappers.right)
            this.wrappers.right.$.style.transform = `translateX(${offset * 100}%)`;
    }
    const SCROLL = 1;
    const SWIPE = 2;
    const SCROLL_X = 3;
    this.setTouchHandlers = (state)=>{
        if (state) {
            ...
            this._.image_container.addEventListener('touchmove', this.touch.moveHandler, {
                passive: false
            });
    ...
    this.touch.moveHandler = e=>{
        ...
        if (this.touch.imagePosition == 0 || this.touch.imagePosition == 1 && this.touch.delta > 0 || this.touch.imagePosition == -1 && this.touch.delta < 0) {
            this.touch.gesture = SCROLL_X;
            return;
        }
        this.touch.deltaY = this.touch.y - this.touch.startY;
        if (Settings.get('lyt.direction') == 'rtl') {
            if ((Reader.SCP.page == Reader.SCP.lastPage && this.touch.delta > 0) || (Reader.SCP.page == 0 && this.touch.delta < 0)) {
        ...
        if (Math.abs(this.touch.delta) > 0.030 || this.touch.gesture == SWIPE) {
            ...
            e.preventDefault();
            if (this.touch.scrollLocked == true)
                return;
        ...
        if (Math.abs(this.touch.deltaY) > this.touch.em) {
            ...
            this.moveWrappers(0);
            if (this.touch.scrollLocked == false)
                return;
    ...
    this.touch.endHandler = e=>{
        if (this.touch.gesture == SCROLL_X || this.touch.gesture == SCROLL)
            return;
        if (this.touch.transitionTimer)
            return;
        ...
        if (isNaN(this.touch.transitionTime))
            this.touch.transitionTime = 0;
        if (isNaN(this.touch.transitionTime))
            debugger ;if ((velocity < this.touch.escapeVelocity * -1 || this.touch.delta < this.touch.escapeDelta * -1) && !(Reader.SCP.chapter == Reader.SCP.lastChapter && Reader.SCP.page == Reader.SCP.lastPage)) {
        ...
        } else if ((velocity > this.touch.escapeVelocity || this.touch.delta > this.touch.escapeDelta) && !(Reader.SCP.chapter == Reader.SCP.firstChapter && Reader.SCP.page == 0)) {
            this.moveWrappers(1, true);
            switch (Settings.get('lyt.direction')) {
            case 'ltr':
                if (this.touch.transitionTimer)
    ...
    this.mouseHandler = function(e) {
        ...
        if (e.type == 'mousemove') {
            if (this.mouseHandler.dead || !this.mouseHandler.active) {
                ...
                if (Math.abs(this.mouseHandler.initPos.x - e.pageX) > 20 || Math.abs(this.mouseHandler.initPos.y - e.pageY) > 20) {
        ...
        if (e.button != 0)
            return;
        if (Settings.get('bhv.clickTurnPage') === false)
            return;
        var box = this.$.getBoundingClientRect();
        var areas = [0, box.width * 0.35 + box.left, box.width * 0.5 + box.left, box.width * 0.65 + box.left, box.width + box.left];
        areas.push(e.pageX);
        areas.sort((a,b)=>a - b);
        if (e.type == 'click') {
            switch (areas.indexOf(e.pageX)) {
            case 1:
                (Settings.get('lyt.direction') == 'rtl') ? this.next(e) : this.prev(e);
                break;
            case 2:
            case 3:
                if (IS_MOBILE)
                ...
                break;
            case 4:
                (Settings.get('lyt.direction') == 'rtl') ? this.prev(e) : this.next(e);
                break;
            default:
                break;
            }
...
function UI_ReaderImageWrapper(o) {
    ...
    this.destroy = ()=>{
        ...
        for (var i = 0; i < children.length; i++) {
            if (children[i]._struct)
                children[i]._struct.destroy();
        }
...
function UI_ReaderNoticeWrapper(o) {
    ...
    function countdown(time) {
        ...
        var times = [Math.floor(t / (1000 * 60 * 60 * 24)) + ' days', Math.floor((t / (1000 * 60 * 60)) % 24) + ' hours', Math.floor((t / 1000 / 60) % 60) + ' minutes'].slice(0, -1);
    ...
    var notice = new UI_Dummy({
        html: ('<div class="ReaderNotice">' + (Reader.current.next_release_html || `<h2>You're caught up!</h2>
			...
			<a href="https://www.viz.com/read/manga/kaguya-sama-love-is-war/all" target="_blank">Buy the official volumes</a>`) + '</div>').replace('$countdown', countdown(release))
    });
...
function URLChanger(o) {
    ...
    this.updateURL = function(SCP) {
        ...
        var pathName = pathSplit.concat([SCP.series, SCP.chapter.replace('.', '-'), SCP.page + 1, '']).join('/');
...
function UI_LodaManager(o) {
    ...
    this.keyListener = new KeyListener(this.$).noPropagation(true).attach('close', ['Escape'], this.close.bind(this))
    this.$.onmousedown = (e)=>{
        if (e.target == this.$) {
            this.close();
        }
...
function UI_Loda(o) {
    ...
    if (o.name)
        this._.header.innerHTML = o.name;
    this.close = function() {
        this.S.out('close');
    }
    this.focus = function() {
        this.focusElement.focus();
        if (this.focusElement.select)
...
function UI_Loda_Search(o) {
    ...
    this.tabs = new UI_Tabs({
        ...
        text: 'Title search'
    })).S.link(this.container).S.linkAnonymous('number', num=>{
        this._.input.focus();
    }
    );
    this.tabs.select(0);
    ...
    if (this.isIndexed) {
        ...
        this.input.S.link(this.indexer).S.linkAnonymous('text', text=>{
            this.tabs.select(1);
...
function UI_Loda_Jump(o) {
    ...
    this.jump = async()=>{
        let chap = this._.input_chap.value || Reader.SCP.chapter
          , page = parseInt(this._.input_page.value) || 1;
        try {
            await Reader.fetchChapter(chap);
            if (page > Reader.current.chapters[chap].images[Reader.getGroup(chap)].length)
                throw "Invalid Chapter or Page!"
    ...
    [this._.input_chap, this._.input_page].forEach(el=>{
        new KeyListener(el).attach('Jump', ['Enter'], e=>this.jump())
    }
    );
}
    ...
    this.search = function(query) {
        ...
        API.requestIndex({
            ...
            for (var searchWord in data.result) {
                ...
                if (!searchResult._merged)
                    searchResult._merged = {};
...
function UI_Loda_Settings(o) {
    ...
    for (let category in Settings.settings) {
        if (Settings.settings[category].hidden)
            continue;
        var container = new UI_Dummy();
        for (let setting in Settings.settings[category]) {
            if (Settings.settings[category][setting].hidden)
                continue;
...
function UI_SettingUnit(o) {
    ...
    this.packetHandler = (packet)=>{
        if (packet.setting != this.setting.addr)
            return;
...
function UI_SettingDisplay(o) {
    ...
    if (this.entity)
        this.$.appendChild(this.entity.$);
    return this;
}
const WIDE_FLAG = "_w.";
function firstPartySeriesHandler(mediaURL, chapter, group, slug) {
    for (let i = 0; i < chapter.groups[group].length; i++) {
        chapter.images[group].push(mediaURL + slug + '/chapters/' + chapter.folder + '/' + group + '/' + chapter.groups[group][i])
        chapter.blurs[group].push(mediaURL + slug + '/chapters/' + chapter.folder + '/' + group + "_shrunk_blur" + '/' + chapter.groups[group][i])
        chapter.previews[group].push(mediaURL + slug + '/chapters/' + chapter.folder + '/' + group + "_shrunk" + '/' + chapter.groups[group][i])
...
function thirdPartySeriesHandler(url, chapter, group) {
    if (Array.isArray(chapter.groups[group])) {
        ...
        chapter.pageRequest[group] = async()=>{
            ...
            try {
                let pages = await fetch(`${chapter.groups[group]}`).then(r=>r.json());
                pages.forEach((p,i)=>{
    ...
    this.downloadChapter = async function() {
        ...
        try {
            ...
            for (let i = 0; i < chapURLArray.length; i += parallelDownloads) {
                if (!continueDownload)
                    return;
                let imageBlobs = await Promise.all(chapURLArray.slice(i, i + parallelDownloads).map((url,subIndex)=>{
                    return (async()=>{
                ...
                imageBlobs.forEach((data)=>{
                    zip.file(data.fileIndex + mimeMap[data.contents.type], data.contents, {
                        binary: true
                    });
                }
                );
